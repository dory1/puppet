...
describe 'vmwaretools::repo', :type => 'class' do
    ...
    redhatish.each do |os|
      describe "for operating system #{os}" do
        ...
        it { should contain_yumrepo('vmware-tools').with(
          ...
          :baseurl         => 'http://unix-prod.bezeqint.net:6163/vmware/rhel6/x86_64/',
          :priority        => '50',
          :protect         => '0',
          :proxy           => 'absent',
    ...
    suseish.each do |os|
      describe "for operating system #{os}" do
        ...
        it { should contain_yumrepo('vmware-tools').with(
          ...
          :gpgcheck        => '0',
          :gpgkey          => "http://packages.vmware.com/tools/keys/VMWARE-PACKAGING-GPG-DSA-KEY.pub\n    http://packages.vmware.com/tools/keys/VMWARE-PACKAGING-GPG-RSA-KEY.pub",
          :baseurl         => 'http://unix-prod.bezeqint.net:6163/vmware/suse10/i586/',
          :priority        => '50',
          :protect         => '0',
          :proxy           => 'absent',
          :proxy_username  => 'absent',
  ...
  context 'on a supported operatingsystem, vmware platform, custom parameters' do
    ...
    describe 'yum_server => http://unix-prod.bezeqint.net:6163' do
      let(:params) {{ :yum_server => 'http://unix-prod.bezeqint.net:6163' }}
      it { should contain_yumrepo('vmware-tools').with(
    ...
    describe 'yum_path => /some/path' do
      ...
      it { should contain_yumrepo('vmware-tools').with(
        ...
        :baseurl  => 'http://unix-prod.bezeqint.net:6163/vmware'
      )}
    end
